...
describe('buildNodeGeneratePayload', () => {
  it('keeps providerRuntime for custom openapi models', () => {
    const payload = buildNodeGeneratePayload({
      prompt: '夕阳美景',
      requestModel: 'custom-openapi:gateway-a:model-main',
      size: '1K',
      aspectRatio: '9:16',
    ...
    expect(payload.model).toBe('custom-openapi:gateway-a:model-main');
  });
});
